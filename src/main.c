...
{
  if (history_limit >= history_capacity)
  {
    history_capacity += 10;
  ...
  history[history_limit++] = strdup(line);
}
...
char **lsh_split_line(char *line)
{
  ...
  while (token != NULL)
  {
    ...
    if (position >= bufsize)
    {
      ...
      // if the reallocation didn't work then we free the old pointer so we don't have any memory leak
